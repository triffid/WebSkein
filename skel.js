...
function findSplitEvent(sv) {
	...
	for (var i = 0, l = skelVertices.length; i < l; i++) {
		...
		// skip the two edges adjacent to sv
		if (cv1 === sv)
			continue;
		if (cv2 === sv)
			continue;
			...
			if (svInvolved.length == 3) {
				...
				if (svInvolved[0].alive && svInvolved[1].alive) {
					...
					if (v2.nextV === v1) {
						...
						v1.startTime = time;
						// what if v2.nextV == v1?
						while (v2.nextV.vector.eql(v1.vector) && v2.nextV.alive) {
							v2.nextV.alive = false;
							v2 = v2.nextV;
						}
						if (v2.nextV.alive) {
				...
				newp.startTime = time;
				// what if p == v1?
				while (v1.vector.eql(p) && v1.alive) {
					v1.alive = false;
					v1 = v1.prevV;
				}
				if (v1.alive) {
					// what if newp == rv.nextV?
					while (rv.nextV.vector.eql(newp.vector) && rv.nextV.alive) {
						rv.nextV.alive = false;
						rv = rv.nextV;
					}
					if (rv.nextV.alive) {
						newp.setPrevV(v1);
						newp.setNextV(rv.nextV);
						while (rv.vector.eql(v2.vector) && v2.alive) {
							v2.alive = false;
							v2 = v2.nextV;
						}
						if (v2.alive) {
